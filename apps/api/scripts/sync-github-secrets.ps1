...
# Root is the backend repo folder. This makes the script work no matter
...
# contain repo name, EC2 key path, and real secrets.
...
if (-not (Test-Path $ProdEnvFile)) { throw "Missing $ProdEnvFile" }
...
# gh secret set EC2_HOST -R arin-codegym/store_springboot
...
# PROD_ENV_FILE is one GitHub secret containing the full production .env file.
# The backend workflow writes it to /home/ubuntu/app/.env on EC2 during deploy.
...
# use file power shell
# cd C:\...\project-root\store-springboot\store
